...
const createDesignation = catchErrors(async (req, res) => {
  ...
  if (existingDesignation) {
    const error = new Error(
      `Designation name '${name}' is already in use. Please use a different name.`
    );
...
const getAllDesignations = catchErrors(async (req, res) => {
  ...
  if (cached) {
    return res
      ...
      .json({
        success: true,
        message: "Designations fetched (cache)",
        designations: cached,
      });
  }
...
const getDesignationById = catchErrors(async (req, res) => {
  ...
  return res
    .status(200)
    .json({ success: true, message: "Designation fetched", designation });
});
...
const updateDesignation = catchErrors(async (req, res) => {
  ...
  // Check for duplicate designation name (excluding current designation)
  if (name) {
  ...
  return res
    .status(200)
    .json({ success: true, message: "Designation updated", designation });
});
