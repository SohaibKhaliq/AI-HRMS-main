...
const TimeTracking = () => {
  ...
  const { activeEntry, myEntries, loading } = useSelector(
  ...
  const calculateWorkedTime = () => {
    ...
    // Subtract break time (server stores break items as startTime / endTime)
    let breakTime = 0;
    if (activeEntry.breaks && activeEntry.breaks.length > 0) {
      activeEntry.breaks.forEach((breakItem) => {
        const startField = breakItem.startTime || breakItem.start; // backward compat
        const endField = breakItem.endTime || breakItem.end;
        if (startField) {
          const breakStart = new Date(startField);
          const breakEnd = endField ? new Date(endField) : now;
  ...
  const isOnBreak = () => {
    if (!activeEntry || !activeEntry.breaks || activeEntry.breaks.length === 0)
      return false;
    const lastBreak = activeEntry.breaks[activeEntry.breaks.length - 1];
    const hasStart = lastBreak && (lastBreak.startTime || lastBreak.start);
    const hasEnd = lastBreak && (lastBreak.endTime || lastBreak.end);
            ...
            <div className="bg-white dark:bg-secondary rounded-lg shadow-md p-6">
              ...
              <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900 dark:bg-opacity-20 rounded-lg border border-blue-200 dark:border-blue-800">
                <div className="flex items-start gap-2 text-sm text-blue-700 dark:text-blue-400">
                  ...
                  <div>
                    <strong>Facial Recognition Required:</strong> All time
                    tracking actions require facial recognition verification for
