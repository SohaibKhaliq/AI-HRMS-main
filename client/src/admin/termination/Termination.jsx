...
import { getTerminations, deleteTermination, updateTermination, createTermination } from "../../services/termination.service";
...
const Termination = () => {
  ...
  const { terminations, loading, error } = useSelector(state => state.termination || { terminations: [], loading: false, error: null });
  ...
  const filtered = useMemo(() => {
    ...
    if (dateFrom) {
      result = result.filter(t => new Date(t.terminationDate) >= new Date(dateFrom));
    }
    if (dateTo) {
      result = result.filter(t => new Date(t.terminationDate) <= new Date(dateTo));
    }
  ...
  const openEdit = (t) => { setAction("update"); setModalOpen(t); };
  const openView = (t) => { setAction("view"); setModalOpen(t); };
  ...
  return (
    <>
      <Helmet><title>Terminations - Metro HR</title></Helmet>
      <section className="bg-gray-100 dark:bg-secondary p-4 rounded-lg min-h-screen shadow">
        ...
        <div className="bg-white rounded-lg shadow p-4 mb-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            <div>
              <label className="text-sm font-medium text-gray-700">Type Filter</label>
              <select value={typeFilter} onChange={e=>{setTypeFilter(e.target.value); setCurrentPage(1);}} className="w-full p-2 border border-gray-300 rounded-lg text-sm">
          ...
          {(typeFilter || statusFilter || dateFrom || dateTo || searchQuery) && (
            <div className="mt-2 text-sm text-blue-600">
              Filters applied: {typeFilter && `Type="${typeFilter}"`} {statusFilter && `Status="${statusFilter}"`} {dateFrom && `From="${dateFrom}"`} {dateTo && `To="${dateTo}"`}
            </div>
          )}
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {pageData?.map((termination, index) => {
                const empName = termination.employee?.name || `${termination.employee?.firstName} ${termination.employee?.lastName}`;
                return (
                  <tr key={termination._id} className="hover:bg-gray-50 transition">
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{(currentPage - 1) * pageSize + index + 1}</td>
                    <td className="px-6 py-4 text-sm text-gray-600">
                      <div className="font-medium">{empName}</div>
                      <div className="text-xs text-gray-500">{termination.employee?.employeeId}</div>
                    </td>
                    <td className="px-6 py-4 text-sm">
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getTypeBadge(termination.type)}`}>
                        {termination.type?.charAt(0).toUpperCase() + termination.type?.slice(1)}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-600">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <select
                        ...
                        onChange={(e) => handleStatusChange(termination, e.target.value)}
                        className={`px-2 py-1 rounded text-xs font-medium border-0 ${getStatusBadge(termination.status)} cursor-pointer`}
                      >
                        <option value="In progress">In progress</option>
                        <option value="Completed">Completed</option>
                        <option value="Cancelled">Cancelled</option>
                    ...
                    <td className="px-6 py-4 text-sm">
                      {termination.documentUrl ? (
                        <a href={termination.documentUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 font-medium flex items-center gap-1">
                          <FaDownload /> View
                        </a>
                      ) : (
                    ...
                    <td className="px-6 py-4 text-sm flex items-center gap-2">
                      <button onClick={() => openView(termination)} className="text-blue-600 hover:text-blue-800 text-lg" title="View">
                        <FaEye />
                      </button>
                      <button onClick={() => openEdit(termination)} className="text-green-600 hover:text-green-800 text-lg" title="Edit">
                        <FaEdit />
                      </button>
                      <button onClick={() => handleDelete(termination._id)} className="text-red-600 hover:text-red-800 text-lg" title="Delete">
                        <FaTrash />
                      </button>
                    </td>
                  </tr>
        ...
        <div className="flex items-center justify-between mt-4 px-4">
          <span className="text-sm text-gray-600">
            Showing {Math.min((currentPage - 1) * pageSize + 1, filtered.length)} to {Math.min(currentPage * pageSize, filtered.length)} of {filtered.length} terminations
          </span>
