...
const JobRequisitions = () => {
    ...
    () => [
      {
        ...
        department: "Human Resources (Downtown Branch)",
        positions: 4,
        status: "Approved",
        priority: "High",
      ...
      },
      {
        ...
        category: "Customer Service",
        department: "Information Technology (Downtown Branch)",
        positions: 1,
        status: "Approved",
        priority: "Medium",
        createdAt: "2025-09-19",
      ...
      },
      {
        ...
        category: "Operations and Management",
        department: "Information Technology (Main Office)",
        positions: 1,
        status: "Approved",
        priority: "High",
        createdAt: "2025-09-19",
      ...
      },
      {
        ...
        department: "Human Resources (Main Office)",
        positions: 1,
        status: "On Hold",
        priority: "Low",
        createdAt: "2025-09-19",
      ...
      },
      {
        ...
        department: "Finance & Accounting (Main Office)",
        positions: 3,
        status: "Approved",
        priority: "High",
        createdAt: "2025-09-19",
  ...
  const handleClose = () => {
    setModalOpen(false);
    if (location.pathname.endsWith("/add"))
      navigate("/recruitment/requisitions", { replace: true });
  };
  ...
  const handleCreateOrUpdate = (form) => {
    if (modalMode === "create") {
      ...
      setItems(
        items.map((it) =>
          it.code === current.code ? { ...current, ...form } : it
        )
      );
    }
    handleClose();
  ...
  const handleApprove = (row) => {
    setItems(
      items.map((it) =>
        it.code === row.code ? { ...it, status: "Approved" } : it
      )
    );
  };
  ...
  const priorityBadge = (priority) => {
    ...
    return (
      <span
        className={`px-2 py-1 rounded-full text-xs font-semibold ${map[priority]}`}
      >
        {priority}
      </span>
    );
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {pageItems.map((row, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 md:px-6 py-3 text-sm text-gray-900 dark:text-white">
                      {row.code}
                    </td>
                    <td className="px-4 md:px-6 py-3 text-sm text-gray-900 dark:text-white">
                      {row.title}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {row.status !== "Approved" && (
                          <button
                            ...
                            className="p-2 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-gray-600 rounded transition"
                            title="Approve"
                          >
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="text-sm text-gray-600 dark:text-gray-400">
            {(() => {
              ...
              return `Showing ${filtered.length ? start : 0} to ${
                filtered.length ? end : 0
              } of ${filtered.length} job requisitions`;
            })()}
          </div>
