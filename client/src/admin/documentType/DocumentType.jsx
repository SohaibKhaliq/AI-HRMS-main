...
import { getDocumentTypes, deleteDocumentType } from "../../services/documentType.service";
import { FaEye, FaEdit, FaTrash, FaPlus } from "react-icons/fa";

const DocumentType = () => {
  const dispatch = useDispatch();
  const { documentTypes, loading, error } = useSelector(state => state.documentType || { documentTypes: [], loading: false, error: null });
  ...
  const filtered = useMemo(() => {
    ...
    return (documentTypes || []).filter(d => d.name.toLowerCase().includes(q) || (d.description||"").toLowerCase().includes(q));
  ...
  const handleDelete = (id) => { if (!confirm("Are you sure?")) return; dispatch(deleteDocumentType(id)); };

  if (error) return <FetchError error={error} />;

  return (
    <>
      <Helmet><title>Document Types - Metro HR</title></Helmet>
      <section className="bg-gray-100 dark:bg-secondary p-4 rounded-lg min-h-screen shadow">
        ...
        <div className="bg-white rounded-lg shadow overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {pageData.length === 0 ? (
                <tr><td colSpan={6} className="px-6 py-4 text-center text-sm text-gray-500">No document types found</td></tr>
              ) : (
                pageData.map((d) => (
                  <tr key={d._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-700">{d.required?"Yes":"No"}</td>
        ...
        <div className="flex items-center justify-between mt-4">
          <div className="text-sm text-gray-600">Showing {pageData.length} of {filtered.length} document types</div>
        ...
        {modalOpen && (
          <DocumentTypeModal action={action} documentType={modalOpen} onClose={()=>setModalOpen(null)} />
        )}
      </section>
    </>
  );
