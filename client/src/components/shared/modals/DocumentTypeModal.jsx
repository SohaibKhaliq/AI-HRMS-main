...
const DocumentTypeModal = ({ action, onClose, documentType }) => {
  ...
  const [formData, setFormData] = useState({ name: "", description: "", required: false, status: "Active", createdAt: "" });
  ...
  useEffect(() => {
    if ((action === "update" || action === "view") && documentType) {
      setFormData({
        ...
        createdAt: documentType.createdAt ? new Date(documentType.createdAt).toISOString().slice(0,16) : "",
      });
    }
  }, [action, documentType]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (action === "update") dispatch(updateDocumentType({ id: documentType._id, docType: formData }));
    ...
    <div className="fixed inset-0 z-40 bg-gray-800 bg-opacity-50 flex justify-center items-center">
      <form onSubmit={handleSubmit} className="bg-white text-black w-[90%] sm:max-w-xl p-6 border border-gray-300 rounded-lg shadow-xl space-y-4">
        <div className="flex justify-between items-center">
          <h3 className="font-bold">{action === 'create' ? 'Create' : action === 'update' ? 'Update' : 'View'} Document Type</h3>
        ...
        <div className="flex items-center gap-4">
          <label className="flex items-center gap-2"><input type="checkbox" name="required" checked={formData.required} onChange={handleChange} disabled={isView} /> Required</label>
          <select name="status" value={formData.status} onChange={handleChange} disabled={isView} className="p-2 bg-[#EFEFEF] rounded-full">
